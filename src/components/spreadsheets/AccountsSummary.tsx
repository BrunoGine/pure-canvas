  ...
  return (
    <div className="glass-card rounded-xl p-4 space-y-3">
      ...
      <div className="space-y-2">
        {accounts.map(acc => (
          <div key={acc.id} className="flex items-center justify-between bg-secondary/30 rounded-lg px-3 py-2.5 border border-border/30">
            <div>
              <p className="text-sm font-medium">{acc.name}</p>
              <p className="text-xs text-muted-foreground capitalize">{acc.account_type.toLowerCase()} • {acc.currency}</p>
            </div>
            <p className={`font-semibold tabular-nums ${Number(acc.balance) >= 0 ? "text-primary" : "text-destructive"}`}>
